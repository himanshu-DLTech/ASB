...
exports.start = async (routeName, rest, _messageContainer, message) => {
    ...
    LOG.info(`[REST] REST call to ${rest.url} with incoming message with timestamp: ${message.timestamp}`);
    ...
    try{
        ...
        let rawData; if(method == "GET") rawData = await fetch(rest.url, {method, headers});
        else rawData = await fetch(rest.url, {method, headers, body: JSON.stringify(message.content)});
        const data = { result: true, data: await rawData.json() };
        message.addRouteDone(`${routeName}`);
        delete message.env[routeName];  // clean up our mess
        message.setGCEligible(true);
        message.content = data;
